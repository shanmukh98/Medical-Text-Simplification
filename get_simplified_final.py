...
from rouge_l import get_rougescore
from flesch_kincaid_score import get_kincaidscore
...
def get_cliner_attributes(line):
	fin = open("./temp_cliner.txt",'w')
	fin.write(line[0]+"\n")
	fin.close()

	os.system("python ./CliNER/cliner predict --txt temp_cliner.txt --out ./ --model ./CliNER/models/silver.crf --format i2b2")
	f = open("./temp_cliner.con",'r')
	line = f.readline()
	phrases = set()
	prefered_names = {}
	while(line!=''):
	...
	# os.system("rm temp_cliner*")
	return phrases,prefered_names

# if __name__=="__main__":
def simplify(input_text, simple_text, medical_text_recognizer, synonym_finder, best_synonym_finder, cnt):
	...
	if(medical_text_recognizer=="metamap"):
		phrases, prefered_names = get_attributes(temp1)
	elif(medical_text_recognizer=="cliner"):
		phrases, prefered_names = get_cliner_attributes(temp1)
	...
	for phrase in phrases:
		...
		best_synonym = ''
		if(best_synonym_finder=='0'):
			best_synonym =  get_best_word_search_results(synonyms)
		elif(best_synonym_finder=='1'):
	...
	print "complex------>",input_text
	print "simple------->",simple_text
	print "pred_simple-->",final_text

	print "---------------Metrics----------------\n\n"

	print "\nBleu Score"
	bleu_score = get_bleuscore(simple_text,final_text)
	print "bleuscore---->",bleu_score

	print "\nRouge_l Score"
	rouge_p,rouge_r,rouge_f = get_rougescore(simple_text, final_text)
	print "Rouge_precision-->",rouge_p
	print "Rouge_Recall----->",rouge_r
	print "Rouge_F_score---->",rouge_f

	print "\nflesch_kincaid_score"
	complex_age,simple_age,predict_age = get_kincaidscore(input_text, simple_text, final_text)
	print "Complex_min_age-->",complex_age
	print "Simple_min_age--->",simple_age
	print "predict_min_Age-->",predict_age
	# for i in synonyms1:
